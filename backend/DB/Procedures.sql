...
DELIMITER //
CREATE PROCEDURE get_personal_expenses(username VARCHAR(255) , start_row INT , end_row INT )
BEGIN

	SELECT activities.activity_id , activities.group_id , 
		activity_expenses.spent , activity_expenses.income , 
        activities.activity_name , activities.activity_type , 
        activities.inserted_by , activities.date_time , activities.is_personal  
        FROM activity_expenses INNER JOIN activities ON activity_expenses.activity_id = activities.activity_id
        WHERE activity_expenses.username = ? LIMIT ?,? ;
...
-- user2 = Username , user2 = pay_to , amount = -ve
-- user2 = Username , user2 = pay_by , amount = +ve
-- user1 = Username , user1 = pay_to , amount = +ve
-- user1 = Username , user1 = pay_by , amount = -ve
SELECT pay_amount as amount , user1 as pay_to , user2 as pay_by FROM to_settle WHERE pay_amount>0 AND (user1='Shaun Phijo' OR user2='Shaun Phijo') AND group_id=2;
SELECT -1*pay_amount as amount , user1 as pay_by , user2 as pay_to FROM to_settle WHERE pay_amount<0 AND (user1='Shaun Phijo' OR user2='Shaun Phijo') AND group_id=1; 
